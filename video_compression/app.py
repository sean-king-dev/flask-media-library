...
import mimetypes  # Import mimetypes module for MIME type checking
...
ALLOWED_EXTENSIONS = set(['txt', 'pdf', 'mp4', 'mov', 'avi', 'mkv', 'ffv1', 'x-ffv1', 'video/x-ffv1', 'lossless'])

def allowed_file(filename):
    mime_type, _ = mimetypes.guess_type(filename)
    return mime_type and mime_type.startswith('video/') and '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS
...
def compress_video(input_path, compression_percentage, lossless=False):
    ...
    if lossless:
        ...
        # Modify the compression parameter format
        compression_param = f'-crf {compression_percentage}'

    command = [ffmpeg_path, '-i', input_path, '-c:v', codec]

    # Add compression parameters only if not lossless
    if not lossless:
        command.extend(compression_param.split())
...
def upload_file():
    if request.method == 'POST':
        ...
        resize_percentage = int(request.form.get('resizePercentage', 50))
        ...
        if video_file and allowed_file(video_file.filename):
            ...
            # Define video_file_path within this scope
